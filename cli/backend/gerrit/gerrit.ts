...
export const fetchGerritData = async (
  config: Config,
  username: string,
  password: string,
...
const getChanges = (
  ...
  return get(
    host,
    cookie,
    `/a/changes/?q=${query}&n=${count}&start=${start}&o=CURRENT_COMMIT&o=CURRENT_REVISION`
  )
}
...
const getChangeDetails = (
  ...
  cookie: string,
  id: number
): Promise<GerritChangeDetail> => {
  return get(host, cookie, `/a/changes/${id}/detail`)
}

const getChangesUntil = async (
  host: string,
  cookie: string,
  query: string
): Promise<GerritChangeSummary[]> => {
  let page = 0
  let changes: GerritChangeSummary[] = []
  while (page < 500) {
...
export const batch = async (
  inputs: (() => Promise<GerritChangeDetail>)[],
  count: number,
  progressFn: (progress: number) => void
): Promise<GerritChangeDetail[]> => {
...
export const getData = async (
  ...
  const promises = changes.map(change => () =>
    getChangeDetails(config.host, cookie, change._number)
  )
