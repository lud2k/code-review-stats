...
const generateRandomReviews = (config: Config, author: string, created: moment.Moment,
                               updated: moment.Moment): Review[] => {
    const ret = []
...
const generateRandomComments = (config: Config, author: string, created: moment.Moment,
                                updated: moment.Moment): Comment[] => {
    const ret = []
    const nbReviews = _.random(MIN_REVIEWS_PER_COMMIT, MAX_REVIEWS_PER_COMMIT)
    for (let j=0; j<nbReviews; j++) {
        const user = getReviewer(author)
