...
export class TableHeader extends React.Component<{ orderBy: OrderBy, columns: TableColumn[],
    ...
    onClickSort = (column: TableColumn) => (event: MouseEvent<HTMLElement>) => {
        ...
        this.props.onChangeOrderBy([orderKey === column.key ? !orderDirection : column.numeric, column.key])
    }
...
export class SortableTable extends React.Component<{ defaultOrderBy: OrderBy, data: TableData, rowsPerPage: number,
    ...
    render() {
        const {data, columns, rowsPerPage, rowRenderer, className} = this.props
